...
function summarize(rows, bankroll = 1000) {
  if (!rows.length) return null;
  const book = Number(bankroll) > 0 ? Number(bankroll) : 1000;
  ...
  // Expectancy as % of the book — lets a $15 book get the same
  // scale_up/base/defensive signals a $1k book would at the same edge.
  const expPct = book > 0 ? expectancy / book : 0;
  ...
  // Small books can't absorb concentration: tighter pct, floored so orders clear
  // the ~$0.40 Polymarket minimum on a $15 book.
  const smallBook = book < 100;
  const maxPositionPct = expectancy > 0
    ? Math.max(smallBook ? 0.2 : 0.04, Math.min(smallBook ? 0.35 : 0.25, 0.06 + expPct * 2))
    : (smallBook ? 0.2 : 0.04);
  ...
  return {
    ...
    expPct: Math.round(expPct * 10000) / 100,
...
function main() {
  const bankrollArg = process.argv.find((a) => a.startsWith('--bankroll='));
  const bankroll = bankrollArg ? Number(bankrollArg.split('=')[1]) : 1000;
  ...
  for (const [k, list] of byStratum.entries()) {
    ...
    const stats = summarize(list, bankroll);
  ...
  const payload = {
    ...
    strata,
    exitMix,
    global: summarize(rows, bankroll),
    notes: [
      ...
      'Re-run nightly or after live sessions: node src/polymarket/heuristics/trainFundHeuristics.js --bankroll=15',
    ],
  };
