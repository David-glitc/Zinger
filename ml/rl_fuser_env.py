...
def _save_steps(steps, path):
    ...
    store_save('ml/rl_fuser_steps.json', data)
    print(f'Saved {len(steps)} steps to sqlite (ml/rl_fuser_steps.json)', flush=True)


def _load_steps(path):
    data = store_load('ml/rl_fuser_steps.json', None)
    if data is None:
        raise FileNotFoundError(f'No fuser steps in sqlite: ml/rl_fuser_steps.json')
    steps = [FuserStep(
        models=[ModelOutput(*m) for m in d['models']],
        actual_return=d['actual_return'], timestamp=d['timestamp'], regime=d['regime'],
    ) for d in data]
    print(f'Loaded {len(steps)} steps from sqlite', flush=True)
    return steps