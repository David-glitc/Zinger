...
# ── 1. Load manifest ────────────────────────────────────────────────
def load_manifest():
    return store_load('ml/models/onnx/manifest.json', [])
...
def backtest_trades():
    ...
    trades = store_load('poly_trades.json', [])
    if not trades:
...
if __name__ == '__main__':
    ...
    store_save('ml/benchmark_results.json', output)
    log(f'\nResults saved to sqlite (ml/benchmark_results.json)')