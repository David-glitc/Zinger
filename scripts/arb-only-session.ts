...
// @ts-nocheck
/**
 * Hard reset + arb-only paper session.
 *
 *   npx tsx scripts/arb-only-session.ts [--bankroll=1000] [--observe] [--target=200]
 */
import path from 'path';
import { fileURLToPath } from 'url';
import { spawn } from 'child_process';
import dotenv from 'dotenv';
import { arbOnlyPaperStrategy } from '../src/polymarket/modeConfig.js';
...
const OBSERVE = args.observe === 'true' || args.observe === true;
const TARGET = Math.max(1, Number(args.target || 200));
const SESSION_CFG = arbOnlyPaperStrategy();
...
async function main() {
  console.log(`\n📦 Arb-only paper session · $${BANKROLL} · ${BASE}\n`);
  ...
  console.log('⚙️  Arb-only config (forceArbOnly, loosened gaps)…');
  ...
  console.log('🧭 Governor → arb-only regime…');
  await api('POST', '/api/poly/governor/regime', { regime: 'arb-only' });
  ...
  console.log(`   forceArbOnly ${SESSION_CFG.forceArbOnly} · minArbGap ${SESSION_CFG.minArbGap} · maxArbPerSlug ${SESSION_CFG.maxArbPerSlug}\n`);

  if (!OBSERVE) return;

  console.log(`👁  Observer target ${TARGET} closes…\n`);
